...
for PLATFORM in $PLATFORMS; do
  ...
  CMD="GOOS=${GOOS} GOARCH=${GOARCH} go build -v -o ${BIN_FILENAME} github.com/clin003/pbbot_app_loginhelper/cmd/pbbot_app_loginhelper"
  echo $CMD
...
if [ -n "$1" ]; then
  rm "$RELEASE_DIR/pbbot_app_loginhelper-$1.zip"
  CMD="zip -r $RELEASE_DIR/pbbot_app_loginhelper-$1.zip $OUTPUT_DIR -x \".*\" -x \"__MACOSX\""
  echo $CMD
  eval $CMD
fi